...
- **Frontend**: React 19 with TypeScript (Vite, dev server on **port 3000**)
- **Auth**: JWT bearer tokens with per-user task isolation (see [Authentication](#authentication))
- **Testing**: xUnit (backend), Vitest + React Testing Library (frontend), Playwright (e2e)
- **CI**: GitHub Actions — fast gate (typecheck + unit/integration), e2e, and Claude AI review (see [Continuous Integration](#continuous-integration))
...
app.UseCors("AllowFrontend");  // Apply before UseAuthentication/UseAuthorization
```
...
The API uses **JWT bearer authentication with per-user task isolation**. Every task is owned by a user; a caller only ever sees and mutates their own tasks. (ADR: `docs/ADR-007-authentication-strategy.md`.)

### How it works

- **Login**: `POST /api/auth/login` (`[AllowAnonymous]`) takes a username + password, verifies the credential with `PasswordHasher<User>` (PBKDF2), and returns a signed JWT (HMAC-SHA256, 60-minute expiry). `/health` is also anonymous; every other endpoint requires a valid token.
- **Seeded users**: users are seeded into the DB via EF `HasData` with pre-computed password hashes — no plaintext passwords in source or DB.
- **Token validation**: `Program.cs` wires `AddAuthentication(JwtBearerDefaults).AddJwtBearer(...)` with full `TokenValidationParameters` (issuer, audience, lifetime, signing key; `ClockSkew = TimeSpan.Zero` so the 1-hour expiry is exact). `app.UseAuthentication()` runs before `app.UseAuthorization()`, both after CORS.
- **Owner from token, never the body**: `TasksController` extracts `userId` from the token claim and passes it into every service call. `CreateTaskAsync` stamps the owner from the token — a client cannot set `UserId` via the request body.

### The 404-not-403 rule (existence non-leak)

When a caller requests a task that is missing **or** owned by another user, the service returns the **same `404 "Task not found"`** for both cases (`TaskService.cs`) — never `403`. A `403` would confirm the task exists, letting an attacker enumerate valid IDs. "Not yours" and "doesn't exist" must be indistinguishable from the client's view. This deviates from a literal "return 403 for forbidden" reading and is intentional; the reasoning lives in ADR-007.

### Signing key & secrets

The JWT signing key comes from configuration (`Jwt` section in `appsettings`/env var), never hardcoded. The dev key is committed for local runs only; production uses a secrets manager (documented in the README).

### Frontend auth flow

- `AuthContext` exposes `user`, `login`, `logout`, `isAuthenticated`; the token is stored in `localStorage` (known XSS trade-off — production migration path to httpOnly cookie / BFF is documented in ADR-007, not built).
- The axios **request interceptor** attaches `Authorization: Bearer <token>`; the **response interceptor** clears the token and redirects to `/login` on a `401` (expired/invalid session).
- `ProtectedRoute` bounces unauthenticated users to `/login`. `Login.tsx` is a Zod-validated form in the Paper layout (left-aligned column, "Ezra" wordmark); on logout, `TaskList` also calls `queryClient.clear()` so the next user never briefly sees the previous user's cached tasks (TanStack Query has a staleTime and isn't user-scoped).
- **Logout** is a client-side token discard — there is no server-side revocation list (documented limitation).
...
public class TodoDbContext : DbContext
{
    ...
    protected override void OnModelCreating(ModelBuilder modelBuilder)
    {
        modelBuilder.Entity<TodoTask>(entity =>
        {
            ...
            entity.HasIndex(e => e.UserId); // Per-user task isolation
        });
        ...
        // Seed users with pre-computed PBKDF2 hashes (no plaintext passwords).
...
- ✅ CI/CD pipeline (GitHub Actions: fast gate, e2e, AI review)
...
- httpOnly-cookie token storage (BFF)

**Note:** Pagination (`getPaged`), authentication, and CI are already implemented.
...
GitHub Actions workflows live in `.github/workflows/`. They enforce — universally and verifiably — the same gates this file describes as advisory.

### Test & build gates

- **`ci.yml` (fast gate)** — runs on every PR and on push to `master`. Two parallel jobs:
  - `frontend`: `npm ci` → `tsc --noEmit` → `npm run coverage` (Vitest). Coverage is reported but **non-blocking**.
  - `backend`: `dotnet restore`/`build` (Release) → `dotnet test` with `--collect:"XPlat Code Coverage"`.
  - Concurrency cancels superseded runs on the same ref.
- **`e2e.yml`** — runs on every PR to `master`. Boots the backend (`dotnet`) + frontend (`npm run dev`) and runs the Playwright suite (`chromium`, serial). Uploads the `playwright-report` artifact on failure. Playwright's `webServer` starts both servers itself when `CI` is set (`reuseExistingServer: !process.env.CI`) and raises the rate-limit permit for the run.

### AI review

- **`claude-code-review.yml`** — general Claude code review on every PR.
- **`claude-security-review.yml`** — dedicated security-focused review (injection, authn/authz, secrets/crypto, data exposure; high-confidence only; excludes DoS/rate-limiting/style).
- **`claude.yml`** — responds to `@claude` mentions in issues/PR comments.
- All four use `anthropics/claude-code-action@v1` authenticated via `${{ secrets.CLAUDE_CODE_OAUTH_TOKEN }}` (the token value lives only in GitHub Secrets, never in git).

### CI vs. these guidelines

These guidelines are advisory and run locally; CI is enforced and runs for everyone on every PR. Treat them as complementary — CI is the backstop when a local gate is skipped. CI proved its worth by surfacing a stale e2e test the moment the gate went live.
...
- ✅ Achieved: Editorial Precision / Paper design system (distinctive UI)
- ✅ Achieved: Comprehensive documentation with ADRs
- ✅ Achieved: Production-ready security, logging, and error handling
- ✅ Achieved: JWT authentication + per-user task isolation (ADR-007)
- ✅ Achieved: CI/CD via GitHub Actions (fast gate, e2e, AI review)
- 🔄 Beyond scope: real-time updates, deployment automation, token revocation
